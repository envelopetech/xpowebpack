...
import { upldate_deal_by_mentioned_users, refused_lead_by_mentioned_users, upldate_deal_by_lose_mentioned_users,get_user_wings_feeds_comments, delete_user_wing_feeds_comments } from '../../../actions/wings/dataactions';
import { lead_status, titleheading, Userwingstabindex, report_form_name,streamcredentials,stream_verb_list, report_type, ButtonType, ButtonText, commonplaceholder, ProfilepicType, ICONS, customPopUp } from '../../../shared/utility';
...
class userwingstreamlineitem extends React.Component {
    ...
    userpostunlikehandler = (event, id) => {
        ...
        this.props.onuserunlikessave(data, this.props.PageType);
    ...
    userpostlikehandler = (event, id) => {
        ...
        this.props.onuserlikessave(data, this.props.PageType);
    ...
    keyPress = (event) => {       
        if (event.keyCode === 13) {
            ...
            this.props.onusercommentssave(data, this.props.PageType);
    ...
    render() {       
        ...
        if (this.props.leadstatusname !== null) {
            divleadstatusname = <div className={classshared.card_label__blue.join(' ')}>{this.props.leadstatusname}</div>
        }
        ...
        if (this.props.tabitems.toString() === Userwingstabindex.conversion.toString()) {
            divposttime = <div className={classshared.flex_flex_align_center_marign_b_m.join(' ')}>
            ...
            if (this.props.lead_status !== null) {
                if (this.props.lead_status.toLowerCase() === lead_status.close.toLowerCase()) {
                    ...
                    leadrejectcommentdiv = <div className={classshared.flex_flex_align_center_marign_b_m.join(' ')}>
                        ...
                        <div className={classshared.seperator_grey.join(' ')}>|</div>
                        <span className={classshared.font_2_regular_text_normal.join(' ')}>Approved by <a href="#/">{this.props.curatorname}</a></span></div>
                }
                        ...
                        <div className={classshared.streamline__card_left.join(' ')}>
                            ...
                            <div className={classshared.streamline__card_header.join(' ')}>
                                <div className={classshared.streamline__card_header_name}>
                                    <span className={classshared.font_2_bold_text_color_purple_margin_r_sm_text_transform_c.join(' ')}>{this.props.userpostername}</span>
                                    <span className={classshared.streamlinepost_type_message.join(' ')}>
                                        {this.props.wall_post_type_header}</span>
                                    {divleadstatusname}
                                </div>
                                {divposttime}
                            </div>
                            ...
                            <div className={classshared.streamline__card_content}>
                                ...
                                <div id={this.props.divborderconversion} className={classshared.lead__card_content}>
                                    ...
                                    <div className={classshared.streamline__card_inner_content}>
                                        ...
                                        <span className={classshared.streamlinepost_type_message.join(' ')}> {this.props.postmessage}</span>
                                    </div>
                                </div>
                                {leadrejectcommentdiv}
                                {
                                    this.props.displayleadcontent && this.props.directmessagedivdisplay ?
                                        (
                                            <div className={classshared.streamline__card_inner_content}>
                                                <span className={classshared.font_1_bold_text_dark.join(' ')}>Contact Details:</span>
                                                <span className={classshared.streamlinepost_type_message.join(' ')}> Will DM</span>
                                            </div>
                                        )
                                        : null
                                }
                                ...
                                <div className={classshared.media_icons}>
                                    ...
                                    <div className={classshared.like_icon_padding.join(' ')}>
                                        <Button btntype={this.props.btnsharecommentlikepost} clicked={this.props.userpostcommenthandler} svgclass={classshared.icon_20_icon_dark_grey_margin_r_10.join(' ')} icon={this.props.postcomments}></Button>
                                        {
                                    ...
                                    <div className={classshared.like_icon_padding.join(' ')}>
                                        <Button btntype={this.props.btnsharecommentlikepost} clicked={this.props.userpostsharehandler} svgclass={classshared.icon_20_icon_dark_grey_margin_r_10.join(' ')} icon={this.props.postshare}></Button>
                                        <span className={classshared.text_light.join(' ')}>13</span>
                                    </div>
                                </div>
                            </div>
                        </div>
...
const mapDispatchToProps = dispatch => {
    return {
        onuserlikessave: (data, pagetype) => dispatch(actions.userlikessave(data, pagetype)),
        onuserunlikessave: (data, pagetype) => dispatch(actions.userunlikessave(data, pagetype)),
        onusercommentssave: (data, pagetype) => dispatch(actions.usercommentssave(data, pagetype)),
        //onuserpostupdate: ( data, pagetype ) => dispatch( actions.userpostupdate( data ) ), 
        //on_feed_post_report_save: ( data ) => dispatch( actions.feed_post_report_save( data ) ), 
    };
};
export default connect(null, mapDispatchToProps)(userwingstreamlineitem, axios);